...
def get_filtered_detections(orchard_name: str, stage_filter: str, period: str) -> List[dict]:
    ...
    for r in records:
        ...
        fruit_per_tree = get_fruit_count({
            "fruit": 0,
            "mature_fruit": r.get("mature_count", 0),
            "immature_fruit": r.get("immature_count", 0),
        }) + r.get("immature_count", 0)
        orchard = get_db().get_orchard(orchard_id) or {}
        fruit = fruit_per_tree * int(orchard.get("tree_count") or 1)
        filtered.append({
            "id": r["id"],
            "date": dt_str,
            "stage": stage_label,
...
(() => {
    ...
    const showCitrusToast = (message, type) => {
        if (!message) return;
        type = type || 'success';
        ...
        window.__citrusToastTimer = setTimeout(() => toast.classList.remove('citrus-toast-show'), 2000);
    };
    window.showCitrusToast = showCitrusToast;
    const bindToastTriggers = () => {
        queryAll('.toast-trigger textarea, #toast_trigger textarea').forEach((ta) => {
            ...
            new MutationObserver(check).observe(ta, { attributes: true, characterData: true, childList: true, subtree: true });
        });
    };
...
def build_history_table_html(orchard_name: str, stage_filter: str, period: str) -> str:
    ...
    if orchard_id is not None:
        for h in get_db().get_history_yields(orchard_id):
            det = h.get('detection_id')
            if det:
                prev = hist_map.get(det)
                if prev is None or str(h.get('recorded_at', '')) > str(prev.get('recorded_at', '')):
                    hist_map[det] = h
    ...
    for r in records:
        ...
        rows.append(
            ...
            f'<td><button type="button" class="hist-del-btn" data-hist-delete="{r["id"]}" onclick="window.__histDeletePendingId={json.dumps(str(r["id"]))};window.triggerHistDelete({json.dumps(str(r["id"]))})">删除</button></td>'
            f"</tr>"
        )
...
def handle_table_delete(record_id_raw, orchard_name, stage_filter, period):
    ...
    if record_id is None:
        ...
        return (
            ...
            (gr.update(value="", visible=False) if gr is not None else ""),
        )

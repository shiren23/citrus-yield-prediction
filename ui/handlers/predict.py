...
def run_prediction(media_path, orchard_name, tree_count, stage_choice, canopy_ratio_pct, model_name):
    ...
    try:
        ...
        # 默认使用模型返回的值作为参考（可能基于 UI 中的树数），但为保证历史/统计与果园登记的树量一致，
        # 我们按果园登记的棵树数计算最终的总产量用于展示与保存。
        total_kg = yield_result["predicted_yield_kg"]
        # 获取果园登记的树量（若果园不存在，上面已用 UI 中的值创建）
        try:
            orchard = db.get_orchard(orchard_id) if db and orchard_id is not None else None
            orchard_tree_count = int(orchard.get("tree_count", int(tree_count or 1))) if orchard else int(tree_count or 1)
        except Exception:
            orchard_tree_count = int(tree_count or 1)
        # 按单棵产量 × 果园总棵数 重新计算要在界面/历史中展示的总产量，保证与历史中果实总数的乘法逻辑一致
        try:
            calc_total_by_tree = round(float(per_tree) * orchard_tree_count, 2)
        except Exception:
            calc_total_by_tree = None
        # 最终用于展示/保存的总产量（优先使用按果园棵树计算的值）
        if calc_total_by_tree is not None:
            total_kg = calc_total_by_tree
        ...
        <div class="info-row"><span class="info-label">果树棵数：</span>{orchard_tree_count}棵</div>
        """
        ...
        """
        if calc_total_by_tree is not None:
            formula_html += f"\n<div class=\"info-row\">按单棵×棵数计算：{calc_total_by_tree} kg（{orchard_tree_count} 棵）</div>"
        ...
        try:
            if stage == "flowering":
                ...
                <div class="calc-card">
                  ...
                  <div class="info-row"><strong>最终预测产量：</strong>{total_kg} kg（{orchard_tree_count} 棵）</div>
                </div>
                """
            elif stage == "immature":
                ...
                calc_html += f"<div class=\"info-row\"><strong>最终预测产量：</strong>{total_kg} kg（{orchard_tree_count} 棵）</div></div>"
            else:
        ...
        state = json.dumps({
            ...
            "stage": stage_info["stage"],
            # 保存时将 predicted_yield 存为按果园棵树计算的总产量，便于历史/统计与果实数一致
            "predicted_yield": total_kg,
...
def save_record_full(state_json: str):
    """保存并返回 state、居中 toast、按钮旁状态。"""
    try:
        state, meta = save_record_core(state_json)
        msg, ok = meta if meta else ("操作完成", True)
    except Exception as e:
        state, msg, ok = state_json, f"保存失败：{e}", False
    cls = "success-text" if ok else "fail-text"
    status_html_value = f'<p class="{cls}">{msg}</p>'
    status = gr.update(value=status_html_value, visible=True) if gr is not None else status_html_value
    return state, toast_output(str(msg), bool(ok)), status

...
def render_history_tab(prediction_state, app_toast):
    ...
    with gr.Tab("历史数据") as history_tab:
        ...
        with gr.Column(elem_classes=["app-card", "form-stack", "history-table-host"]):
            ...
            hist_delete_status = gr.HTML("", visible=False)
            ...
            # Refresh orchard dropdown when navigating to History tab.
            # Use a server-side refresh with no component input to avoid
            # passing complex component state through the tab select event
            # which can cause the UI to hang in some Gradio versions.
            def _refresh_orchard_on_select():
                return make_orchard_dropdown_update(None, default_orchard_name())

            history_tab.select(
                _refresh_orchard_on_select,
                inputs=[],

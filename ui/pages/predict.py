...
def render_prediction_tab(prediction_state, app_toast, history_tab=None):
    with gr.Tab("产量预测") as predict_tab:
        ...
        with gr.Column(
            ...
            with gr.Row(elem_classes=["row-compact", "layout-full"], equal_height=True):
                ...
                with gr.Column(scale=1, elem_classes=["app-card", "form-stack"]):
                    ...
                    # 新增：预测计算过程展示卡片（逐步列出输入参数与计算公式）
                    calc_steps = gr.HTML("")
                    yield_range = gr.HTML("")
                    risk_block = gr.HTML(visible=False)
                    with gr.Row(elem_classes=["btn-row"]):
                        save_btn = gr.Button("保存记录", variant="secondary", scale=1)
                        # 直接把导出按钮实现为 DownloadButton，点击时后端返回临时文件路径用于下载
                        export_btn = gr.DownloadButton("导出", variant="secondary")
        ...
        # 避免在首次打开标签页时触发果园下拉框的自刷新（可能导致 Gradio 在
        # 组件尚未完全挂载时卡住）。果园下拉会在新增/删除操作后通过各自的
        # 更新路径（见 `ui/pages/config.py` 中的 dropdown 更新）被刷新。
